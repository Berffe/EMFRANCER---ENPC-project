...
## ComputerVisionTraining
Code responsible for the neural network training model. The base model is a YOLO v8n trained on the default datasets and later converted in a NCNN model (more adjusted for embarked systems).

## GroundStation
Code responsible for the image reconstruction based on the annotations in JSONL during the mission. Biggest troubleshooting and parameter tuning tool.

## MavlinkComm
All the code that goes into the RaspberryPi - decision architecture, communication protocols, video saving and image recognition. The complete architecture is explained in further detail in the folder itself